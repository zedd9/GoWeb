...
func TestCreatUser(t *testing.T) {
	...
	resp, err := http.Post(ts.URL+"/users", "application/json",
		strings.NewReader(`{"first_name":"zedd", "last_name":"shin", "email":"shu2468@naver.com"}`))
...
func TestUpdateUser(t *testing.T) {
	...
	updateStr := fmt.Sprintf(`{"id":%d, "first_name":"updated", "last_name":"YYY"}`, user.ID)
	...
	assert.Equal("YYY", updateUser.LastName)
...
func TestUsers_WithUsersData(t *testing.T) {
	...
	resp, err := http.Post(ts.URL+"/users", "application/json",
		strings.NewReader(`{"first_name":"zedd", "last_name":"shin", "email":"zedd@naver.com"}`))
	assert.NoError(err)
	assert.Equal(http.StatusCreated, resp.StatusCode)
